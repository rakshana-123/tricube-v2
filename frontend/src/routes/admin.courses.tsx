...
export const Route = createFileRoute("/admin/courses")({
  head: () => ({
    meta: [{ title: "Admin · Courses — TRI CUBE" }, { name: "robots", content: "noindex" }],
  }),
  ...
  return (
    <>
      <PageHero
        ...
        title={
          <>
            Manage <span className="gold-text">Courses</span>
          </>
        }
...
function LoginForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    ...
    >
      ...
      <button
        disabled={busy}
        className="w-full rounded-lg bg-[var(--gradient-gold)] px-4 py-2 font-medium text-black disabled:opacity-60"
      >
...
function CoursesManager({ onSignOut }: { onSignOut: () => void }) {
  ...
  const filtered = useMemo(() => {
    ...
    return courses.filter(
      (c) => c.title.toLowerCase().includes(t) || c.slug.toLowerCase().includes(t),
    );
  }, [q, courses]);
    ...
    <div>
      <div className="mb-6 flex flex-wrap items-center gap-3">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg bg-[var(--gradient-gold)] px-4 py-2 text-sm font-medium text-black"
        >
                  ...
                  <tr key={c.id} className="border-t border-border">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        {c.thumbnailUrl ? (
                          <img
                            src={absoluteMedia(c.thumbnailUrl)}
                            alt=""
                    ...
                    <td className="px-4 py-3">
                      {c.modules.length} · {videoCount} videos
                    </td>
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs ${c.published ? "bg-green-500/10 text-green-700" : "bg-muted text-muted-foreground"}`}
                      >
                        {c.published ? (
                          <>
...
function CourseEditor({
  ...
  const [c, setC] = useState<AdminCourse>(
    initial
      ? { ...initial, modules: initial.modules.map((m) => ({ ...m, videos: [...m.videos] })) }
      : emptyCourse(),
  );
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState<string | null>(null);
  ...
  function set<K extends keyof AdminCourse>(k: K, v: AdminCourse[K]) {
    setC((p) => ({ ...p, [k]: v }));
  }
      ...
      modules: p.modules.map((m, idx) =>
        idx === mi
          ? {
              ...
              videos: [
                ...m.videos,
                { title: "New video", url: "", durationSec: 0, isFreePreview: false },
              ],
            }
          : m,
      ),
  ...
  function removeVideo(mi: number, vi: number) {
    setC((p) => ({
      ...p,
      modules: p.modules.map((m, idx) =>
        idx === mi ? { ...m, videos: m.videos.filter((_, j) => j !== vi) } : m,
      ),
    }));
  }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div
        className="absolute right-0 top-0 h-full w-full max-w-3xl overflow-y-auto bg-background shadow-2xl"
      ...
      >
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-background/95 px-6 py-4 backdrop-blur">
          ...
          <button onClick={onClose} className="rounded p-2 hover:bg-muted/50">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="space-y-6 p-6">
          ...
          <div className="rounded-2xl border border-border bg-card p-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button
                onClick={addModule}
                className="inline-flex items-center gap-1 rounded-lg bg-[var(--gradient-gold)] px-3 py-1.5 text-sm font-medium text-black"
              >
                  ...
                  <div className="space-y-2 pl-4">
                    {m.videos.map((v, vi) => (
                      <div key={vi} className="grid grid-cols-12 items-center gap-2">
                        ...
                        <input
                          type="number"
                          value={v.durationSec ?? 0}
                          onChange={(e) =>
                            updateVideo(mi, vi, { durationSec: Number(e.target.value) })
                          }
                          className="input col-span-1"
                          title="Duration (sec)"
                        />
                        <label
                          className="col-span-1 flex items-center gap-1 text-xs"
                          title="Free preview"
                        >
                          <input
                            type="checkbox"
                            ...
                            onChange={(e) =>
                              updateVideo(mi, vi, { isFreePreview: e.target.checked })
                            }
                          />
                          Free
                        </label>
                    ...
                    <button
                      onClick={() => addVideo(mi)}
                      className="inline-flex items-center gap-1 rounded-lg border border-dashed border-border px-3 py-1.5 text-xs text-muted-foreground hover:bg-muted/40"
                    >
        ...
        <div className="sticky bottom-0 flex justify-end gap-2 border-t border-border bg-background/95 px-6 py-4 backdrop-blur">
          ...
          <button
            disabled={busy}
            onClick={handleSave}
            className="rounded-lg bg-[var(--gradient-gold)] px-5 py-2 text-sm font-medium text-black disabled:opacity-60"
          >
            {busy ? "Saving…" : isNew ? "Create course" : "Save changes"}
          </button>
        </div>
      </div>
    </div>
