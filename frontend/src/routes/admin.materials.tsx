...
export const Route = createFileRoute("/admin/materials")({
  head: () => ({
    meta: [{ title: "Admin · Materials — TRI CUBE" }, { name: "robots", content: "noindex" }],
  }),
  ...
  return (
    <>
      <PageHero
        ...
        title={
          <>
            Manage <span className="gold-text">Materials</span>
          </>
        }
        subtitle="Add, edit, upload PDFs and covers, and build bundles."
      />
    ...
    <>
      <div className="mb-6 flex items-center justify-between">
        <div className="flex gap-2">
          <button onClick={() => setTab("materials")} className={tabCls(tab === "materials")}>
            <FileText className="h-3.5 w-3.5" /> Materials
          </button>
          <button onClick={() => setTab("bundles")} className={tabCls(tab === "bundles")}>
            <Package className="h-3.5 w-3.5" /> Bundles
          </button>
          <button onClick={() => setTab("orders")} className={tabCls(tab === "orders")}>
            <Activity className="h-3.5 w-3.5" /> Orders
          </button>
        </div>
        <button
          onClick={onSignOut}
...
const tabCls = (active: boolean) =>
  `inline-flex items-center gap-1 rounded-full px-4 py-2 text-xs font-semibold ${active ? "bg-[var(--gradient-gold)] text-primary-foreground" : "border border-border bg-background"}`;
...
function MaterialsTab() {
  ...
  async function reload() {
    ...
    setErr(null);
    try {
      setRows(await adminListMaterials());
    } catch (e: any) {
      setErr(e.message);
    } finally {
      setLoading(false);
              ...
              {rows.map((m) => (
                <tr key={m.id} className="border-t border-border">
                  <td className="p-3">
                    {m.coverUrl ? (
                      <img
                        src={absoluteMedia(m.coverUrl)}
                        alt=""
                        className="h-10 w-16 rounded object-cover"
                      />
                    ) : (
                      <div className="grid h-10 w-16 place-items-center rounded bg-secondary">
                  ...
                  <td className="p-3">
                    <div className="font-medium">
                      {m.title}{" "}
                      {m.featured && (
                        <Star className="ml-1 inline h-3 w-3 fill-primary text-primary" />
                      )}
                    </div>
                  ...
                  <td className="p-3 text-center">
                    <button
                      onClick={async () => {
                        await adminToggleMaterial(m.id!);
                        reload();
                      }}
                      className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-semibold ${m.active ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}
                    >
...
function MaterialDialog({
  ...
  const [f, setF] = useState({
    slug: initial?.slug || "",
    title: initial?.title || "",
    category: initial?.category || "Interview Prep",
    price: initial?.price?.toString() || "199",
    pages: initial?.pages?.toString() || "20",
    rating: initial?.rating?.toString() || "4.9",
    description: initial?.description || "",
    longDescription: initial?.longDescription || "",
    highlights: (initial?.highlights || "").replaceAll("|", "\n"),
  ...
  async function submit() {
    ...
    try {
      ...
      if (initial?.id) await adminUpdateMaterial(initial.id, fd);
      else await adminCreateMaterial(fd);
      onSaved();
    } catch (e: any) {
      setErr(e.message);
    } finally {
      ...
      >
        ...
        <Field label="Long description">
          <textarea
            ...
            onChange={(e) => setF({ ...f, longDescription: e.target.value })}
          />
        </Field>
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-3">
          ...
          <FileField
            ...
            existing={initial?.pdfUrl}
          />
          <FileField
            label="Preview PDF (free)"
            file={preview}
            onFile={setPreview}
            accept="application/pdf"
            existing={initial?.previewUrl}
          />
        </div>
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <button
            disabled={busy}
            onClick={submit}
            className="rounded-full bg-[var(--gradient-gold)] px-5 py-2 text-xs font-semibold text-primary-foreground disabled:opacity-60"
          >
            {busy ? "Saving…" : "Save"}
          </button>
        </div>
...
function OrdersTab() {
  ...
  async function reload() {
    ...
    try {
      setRows(
        await adminListMaterialOrders({ q: q.trim() || undefined, status: status || undefined }),
      );
    } catch (e: any) {
      setErr(e.message);
    } finally {
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <input
          ...
          placeholder="Search email, slug, Razorpay id…"
                ...
                <>
                  <tr key={o.id} className="border-t border-border align-top">
                    ...
                    <td className="p-3 text-xs">
                      <span className="rounded-full bg-secondary px-2 py-0.5 text-[10px] uppercase tracking-widest">
                        {o.kind}
                      </span>
                      <div className="mt-1 font-medium">{o.slug}</div>
                    </td>
                    <td className="p-3 text-right text-xs">₹{o.amount.toLocaleString()}</td>
                    <td className="p-3 text-center">
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-widest ${map[status] || "bg-secondary"}`}
    >
      {status}
    </span>
  );
...
function Timeline({ order }: { order: OrderRow }) {
  ...
  const stages: OrderEvent[] = [
    {
      at: order.createdAt,
      type: "order.created",
      message: `Razorpay order ${order.razorpayOrderId} created`,
    },
    ...order.events,
  ];
  if (order.paidAt && !order.events.some((e) => e.type === "webhook.paid")) {
    stages.push({
      at: order.paidAt,
      type: "webhook.paid",
      message: `Marked paid${order.razorpayPaymentId ? ` (${order.razorpayPaymentId})` : ""}`,
    });
  }
  if (order.downloadTokenIssuedAt && !order.events.some((e) => e.type.startsWith("download."))) {
    stages.push({
      at: order.downloadTokenIssuedAt,
      type: "download.issued",
      message: "Signed 24h download URL issued",
    });
  }
  ...
  return (
    <ol className="relative ml-2 border-l border-border">
      ...
      {stages.length === 0 && (
        <li className="ml-4 text-xs text-muted-foreground">No events recorded.</li>
      )}
    </ol>
  );
}
...
function EventIcon({ type }: { type: string }) {
  ...
  if (type === "verify.failed" || type === "webhook.failed")
  ...
  if (type === "webhook.duplicate" || type === "webhook.ignored")
  ...
  return (
    <label className="block cursor-pointer rounded-xl border border-dashed border-border bg-background/60 p-4 text-center text-xs">
      ...
      <div className="mt-1 text-muted-foreground">
        {file ? file.name : existing ? "Replace file" : "Choose file"}
      </div>
      <input
        type="file"
        accept={accept}
...
function BundlesTab() {
  const [rows, setRows] = useState<BundleDTO[]>([]);
  ...
  async function reload() {
    ...
    setErr(null);
    try {
      const [b, m] = await Promise.all([adminListBundles(), adminListMaterials()]);
      setRows(b);
      setMaterials(m);
    } catch (e: any) {
      setErr(e.message);
        ...
        <div className="grid gap-3">
          {rows.map((b) => (
            ...
            >
              {b.coverUrl ? (
                <img
                  src={absoluteMedia(b.coverUrl)}
                  alt=""
                  className="h-14 w-24 rounded object-cover"
                />
              ) : (
                <div className="grid h-14 w-24 place-items-center rounded bg-secondary">
              ...
              <div className="flex-1">
                <div className="font-semibold">
                  {b.title}{" "}
                  {b.featured && <Star className="ml-1 inline h-3 w-3 fill-primary text-primary" />}
                </div>
                <div className="text-xs text-muted-foreground">
                  {b.slug} · {(b.items || []).length} items · ₹{b.price.toLocaleString()}
                </div>
              </div>
              <button
                onClick={async () => {
                  await adminToggleBundle(b.id!);
                  reload();
                }}
                className={`rounded-full px-2 py-1 text-[10px] font-semibold ${b.active ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}
              >
...
function BundleDialog({
  ...
  const [selectedIds, setSelectedIds] = useState<number[]>(
    (initial?.items?.map((it) => (it.material as any).id).filter(Boolean) as number[]) || [],
  );

  const originalTotal = useMemo(
    () => materials.filter((m) => selectedIds.includes(m.id!)).reduce((s, m) => s + m.price, 0),
  ...
  async function submit() {
    ...
    try {
      ...
      if (cover) fd.append("cover", cover);
      if (initial?.id) await adminUpdateBundle(initial.id, fd);
      else await adminCreateBundle(fd);
      onSaved();
    } catch (e: any) {
      setErr(e.message);
    } finally {
        ...
        <div className="mt-3">
          <div className="mb-2 flex items-center justify-between text-xs">
            ...
            <span className="text-muted-foreground">
              Total if bought individually: ₹{originalTotal.toLocaleString()}
            </span>
          </div>
            ...
            {materials.map((m) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground">₹{m.price.toLocaleString()}</span>
                </label>
              );
            })}
            {!materials.length && (
