  ...
  if (loading && !stats) {
    return (
      <div className="flex items-center gap-2 rounded-2xl border border-border bg-card p-6 text-sm text-muted-foreground shadow-sm">
        <Loader2 className="h-4 w-4 animate-spin" /> Loading live counts from backend…
      </div>
    );
  }
  ...
  if (err) {
    return (
      <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-sm text-red-700">
        ...
        <button
          onClick={load}
          className="ml-3 inline-flex items-center gap-1 rounded-md border border-red-300 bg-white px-2 py-1 text-xs font-semibold text-red-700"
        >
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={load}
          className="inline-flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-1.5 text-xs font-semibold hover:border-[var(--gold)]"
        >
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <RevenueCard
          label="Courses"
          value={stats.courseRevenue}
          icon={<BookOpen className="h-5 w-5" />}
        />
        <RevenueCard
          label="Materials"
          value={stats.materialRevenue}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          value={stats.materialOrdersPaid}
          sub="paid"
        />
        <StatCard
          ...
          sub={`${stats.pendingRequests} open`}
        />
        <StatCard
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <RecentPanel title="Recent service requests">
            {recent.requests.length === 0 ? (
              ...
              recent.requests.map((r: any) => (
                <RecentRow
                  key={r.id}
                  title={r.serviceTitle}
                  sub={`${r.name} · ${r.email}`}
                  right={
                    <span className="rounded-full bg-[var(--gold-soft)] px-2 py-0.5 text-[10px] font-semibold uppercase text-[var(--gold-dark)]">
                      {r.status}
                    </span>
                  }
                  date={r.createdAt}
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
      <div className="flex items-center justify-between">
        ...
        <span className="grid h-8 w-8 place-items-center rounded-lg bg-[var(--gold-soft)] text-[var(--gold-dark)]">
          {icon}
        </span>
      </div>
...
function RevenueCard({
  ...
  return (
    <div
      className={`rounded-2xl border p-5 shadow-sm ${highlight ? "border-[var(--gold)] bg-[var(--gold-soft)]" : "border-border bg-card"}`}
    >
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
          {label}
        </span>
        <span className="grid h-8 w-8 place-items-center rounded-lg bg-white text-[var(--gold-dark)]">
          {icon}
        </span>
      </div>
      <div className="mt-3 text-3xl font-semibold tracking-tight text-[var(--gold-dark)]">
        {fmtINR(value)}
      </div>
