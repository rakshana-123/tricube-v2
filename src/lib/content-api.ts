...
export async function listTestimonials(): Promise<TestimonialDTO[]> {
  ...
  return rows.filter((t) => t.published).sort((a, b) => b.id - a.id);
}
...
export async function adminUpdateGallery(
  id: number,
  input: Partial<GalleryDTO>,
): Promise<GalleryDTO> {
  const r = await adminFetch(
    `/api/content/gallery/${id}`,
...
export async function adminListTestimonials(): Promise<TestimonialDTO[]> {
  const r = await adminFetch(
    "/api/content/admin/testimonials",
    { headers: authHeaders() },
