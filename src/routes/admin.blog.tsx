...
export const Route = createFileRoute("/admin/blog")({
  head: () => ({
    meta: [
      { title: "Blog — Admin — TRI CUBE" },
      {
        name: "description",
        content: "Write, edit and publish blog posts shown on the public blog page.",
      },
  ...
  component: () => (
    <BackendStatusProvider>
      <PageHero
        ...
        title={
          <>
            Manage <span className="gold-text">Blog</span>
          </>
        }
      ...
      {draft && (
        <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
          <h3 className="mb-4 text-lg font-semibold">
            {draft._isNew ? "New post" : `Edit — ${draft.title}`}
          </h3>
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Field label="Content (HTML or Markdown)" full>
              <textarea
                rows={10}
          ...
          <div className="mt-6 flex justify-end gap-3">
            ...
            <button
              onClick={save}
              disabled={busy || !online || !draft.title || !draft.slug}
              className="inline-flex items-center gap-2 rounded-lg bg-[var(--gradient-gold)] px-4 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-60"
            >
