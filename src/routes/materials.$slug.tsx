...
export const Route = createFileRoute("/materials/$slug")({
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        { title: m ? `${m.title} — TRI CUBE Materials` : "Material — TRI CUBE" },
        {
          name: "description",
          content: m ? `${m.description} · ${m.pages} pages PDF.` : "TRI CUBE study material.",
        },
      ],
    };
  },
...
function MaterialDetail() {
  ...
  const previewHref = material.previewUrl
    ? `${API_BASE}/api/materials/preview/${encodeURIComponent(material.slug)}`
    : null;
  const unlockedHref = (() => {
  ...
  const galleryLabel = owned ? "Your library · full document" : "Free sample pages";

  useEffect(() => {
    setOwned(isOwned(material.slug));
  }, [material.slug]);
  ...
  async function handleBuy() {
      ...
      onPaid: async (r) => {
        ...
        while (Date.now() < deadline) {
          ...
          if (data?.ok && data?.downloadUrl) {
            setDownloadUrl(material.slug, data.downloadUrl);
            return;
          }
          if (data?.pending) {
            await new Promise((s) => setTimeout(s, 2000));
        ...
        throw new Error(
          "Payment received. Confirmation is taking longer than usual — check your email for the download link.",
        );
      },
    });
    setBusy(false);
    if (res.status === "paid") {
      ...
      setOwned(true);
      setTimeout(() => {
        triggerDownload(material.slug, material.title).then((r) => {
          if (!r.ok) setErr(r.error);
        });
      }, 250);
      setTimeout(() => {
      ...
      setErr(
        res.message || "Demo mode is enabled — no real payment was taken, so the PDF stays locked.",
      );
  ...
  return (
    <>
      <PageHero
        ...
        title={<>{material.title}</>}
        subtitle={`${material.pages} pages · Instant PDF download`}
      />
        ...
        <div>
          {demo && (
            <div className="mb-3 rounded-xl border border-primary/30 bg-primary/10 px-4 py-2 text-xs text-foreground">
              Demo mode — backend offline. Sample PDF delivered for preview.
            </div>
          )}
          ...
          <div className="mt-8">
            ...
            <p className="mt-3 text-sm text-muted-foreground whitespace-pre-line">
              {material.longDescription || material.description}
            </p>
            {highlights.length > 0 && (
              <>
        ...
        <aside className="glass h-fit rounded-2xl p-6">
          <div className="flex items-center justify-between text-xs uppercase tracking-widest text-muted-foreground">
            ...
            <span className="flex items-center gap-1">
              <Star className="h-3 w-3 fill-primary text-primary" /> {material.rating}
            </span>
          </div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {material.pages}-page PDF · lifetime re-download
          </p>
          <button
            onClick={handleBuy}
            disabled={busy}
          ...
          >
            ...
            {owned ? "Download PDF again" : busy ? "Opening checkout…" : "Buy & download PDF"}
          </button>
          {owned && !busy && (
            <p className="mt-3 text-center text-xs text-primary">
              You already own this — download unlocked.
            </p>
          )}
          {err && !owned && (
            <PaymentRetry
          ...
          <p className="mt-3 text-center text-xs text-muted-foreground">
            <Lock className="mr-1 inline h-3 w-3" /> Secure payment via Razorpay. Saved to My
            Materials.
          </p>
          <Link
            to="/my-materials"
            className="mt-4 block text-center text-xs font-medium text-foreground/70 underline-offset-4 hover:underline"
          >
            View my library →
          </Link>
        </aside>
