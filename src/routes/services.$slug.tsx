...
import { getService, splitFeatures, absoluteMedia, parseSections, type ServiceDTO, type ServiceSection } from "@/lib/services-api";
import { CheckCircle2, Upload, Lock, ArrowLeft, CreditCard, FileUp, UserCheck, Mail, Clock, Sparkles } from "lucide-react";
...
export const Route = createFileRoute("/services/$slug")({
  head: ({ params }) => ({
    meta: [
      { title: `${params.slug} â€” TRI CUBE Services` },
      { name: "description", content: "TRI CUBE service request â€” pay securely, submit your details, and our team delivers to your email." },
    ],
  }),
...
function ServiceDetail() {
  ...
  useEffect(() => {
    let alive = true;
    getService(slug).then((r) => { if (alive) { setSvc(r); setLoading(false); } });
    return () => { alive = false; };
  }, [slug]);
    ...
    try {
      const res = await startPayment({
        ...
        onPaid: async (resp) => {
          ...
          if (!r.ok) throw new Error("Payment succeeded but submission failed. Our team will contact you.");
        },
      });
      if (res.status === "paid") setDone(true);
      else if (res.status === "demo") setError(res.message || "Demo mode is enabled â€” no real payment was taken, so the service is not activated.");
      else if (res.status === "cancelled") setError(res.message || "Payment was cancelled before it completed.");
      else if (res.status === "failed") setError(res.message || "Payment failed. Please try again.");
    } catch (err: any) {
  ...
  if (loading) {
    return <div className="mx-auto max-w-3xl px-6 py-24 text-center text-sm text-muted-foreground">Loadingâ€¦</div>;
  }
  if (!svc) {
    return (
      <div className="mx-auto max-w-3xl px-6 py-24 text-center">
  ...
  return (
    <>
      <PageHero
        eyebrow={isResumeReview ? "Resume Review Â· â‚¹99" : "Service request"}
        title={<>{svc.title}</>}
        subtitle={
          isResumeReview
            ? "Pay â‚¹99, upload your resume, and get a detailed line-by-line review from our expert â€” emailed to you in 3â€“4 working days."
      ...
      {isResumeReview && (
        <section className="mx-auto max-w-6xl px-6">
          <div className="grid gap-3 rounded-2xl border border-border bg-card p-4 sm:grid-cols-4">
            {[
              { icon: CreditCard, title: "1. Pay â‚¹99", body: "Secure Razorpay checkout." },
              { icon: FileUp,     title: "2. Upload resume", body: "PDF or DOCX, up to 10 MB." },
              { icon: UserCheck,  title: "3. Expert reviews", body: "Our reviewer works on it in 3â€“4 working days." },
              { icon: Mail,       title: "4. Emailed back", body: `Reviewed file + notes to your inbox by ${expected}.` },
            ].map((s) => (
      ...
      <section className="mx-auto grid max-w-6xl gap-8 px-6 py-16 md:grid-cols-[1fr_1.2fr]">
        <aside className="glass h-fit rounded-2xl p-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Duration Â· {svc.duration}</p>
          ...
          <div className="mt-6 flex items-end gap-2">
            <div className="text-3xl font-semibold teal-text">â‚¹{priceOffer.toLocaleString()}</div>
            {svc.price > priceOffer && (
              <div className="pb-1 text-xs text-muted-foreground line-through">â‚¹{svc.price.toLocaleString()}</div>
            )}
          </div>
          {isResumeReview && (
          ...
          <p className="mt-4 flex items-center gap-2 text-xs text-muted-foreground">
            <Lock className="h-3 w-3" /> Secure Razorpay checkout Â· UPI, cards, netbanking
          </p>
        </aside>
        ...
        {done ? (
          <div className="glass rounded-2xl p-8 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              {demo
                ? <>Demo mode â€” backend offline. Connect the local backend to actually deliver to <b>{form.email}</b>.</>
                : isResumeReview
                  ? <>Payment confirmed. Your reviewed resume will be emailed to <b>{form.email}</b> by <b>{expected}</b>.</>
                  : <>Payment confirmed. Our team will review your request and email the deliverable to <b>{form.email}</b> shortly.</>}
            </p>
            <Link to="/services" className="mt-6 inline-flex rounded-full bg-[var(--gradient-gold)] px-5 py-2.5 text-sm font-semibold text-primary-foreground">
              Browse more services
            </Link>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="glass rounded-2xl p-6 md:p-8">
            <h3 className="text-lg font-semibold">{isResumeReview ? "Your details & resume" : "Your details"}</h3>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              ...
              <Field label={isResumeReview ? "Phone (WhatsApp preferred)" : "Phone"}><input value={form.phone} onChange={(e) => set("phone", e.target.value)} className={inputCls} /></Field>
              <Field label="Target role / company"><input value={form.targetRole} onChange={(e) => set("targetRole", e.target.value)} className={inputCls} placeholder="e.g. SDE-1, Fintech" /></Field>
            </div>
            <Field label={isResumeReview ? "What should the reviewer focus on?" : "Notes / requirements"}>
              <textarea rows={4} value={form.notes} onChange={(e) => set("notes", e.target.value)} className={inputCls + " resize-none"} placeholder={isResumeReview ? "e.g. Applying for SDE-1 roles. Please improve impact statements, ATS score and formatting." : "Tell us what you need. Links, references, deadlinesâ€¦"} />
            </Field>
            <Field label={isResumeReview ? "Upload your resume * (PDF or DOCX, max 10 MB)" : "Attach file (resume, brief, references â€” max 10 MB)"}>
              <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-dashed border-border bg-card px-4 py-3 text-sm hover:bg-secondary">
                ...
                <span className="flex-1 truncate">{form.file?.name || (isResumeReview ? "Click to upload your resume (PDF or DOCX)" : "Click to choose a file (PDF, DOCX, ZIP, images)")}</span>
                <input
                  type="file"
                  className="hidden"
                  onChange={(e) => set("file", e.target.files?.[0] || null)}
                  accept={isResumeReview ? ".pdf,.doc,.docx" : ".pdf,.doc,.docx,.zip,.png,.jpg,.jpeg,.txt"}
                />
              </label>
            </Field>
            ...
            >
              {submitting ? "Opening secure checkoutâ€¦" : `Pay â‚¹${priceOffer.toLocaleString()} & submit`}
            </button>
            <p className="mt-3 text-center text-xs text-muted-foreground">
              {isResumeReview
                ? <>Pay first via Razorpay. Your resume is sent to our expert and the review is emailed back within 3â€“4 working days.</>
                : <>You'll pay first via Razorpay. Your details + file are then sent to our team for review and email delivery.</>}
            </p>
          </form>
        )}
      </section>
...
function SectionBlock({ section }: { section: ServiceSection }) {
  switch (section.type) {
    ...
    case "paragraph":
      return <p className="max-w-3xl text-sm leading-relaxed text-muted-foreground md:text-base">{section.text}</p>;
    case "steps":
      return (
        <div>
          ...
          <ol className="grid gap-3 md:grid-cols-2">
            {section.items.map((it, i) => (
              <li key={i} className="flex gap-3 rounded-2xl border border-border bg-card p-4">
                <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[var(--gradient-gold)] text-sm font-semibold text-primary-foreground">{i + 1}</span>
                <div>
        ...
        <div>
          ...
          <ul className="grid gap-2 md:grid-cols-2">
            {section.items.map((t, i) => (
              <li key={i} className="flex items-start gap-2 rounded-xl bg-secondary/50 px-4 py-3 text-sm">
          ...
          <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-4">
            {section.items.map((h, i) => (
              <div key={i} className="rounded-2xl border border-border bg-card p-4">
                <div className="text-2xl font-semibold teal-text">{h.value}</div>
